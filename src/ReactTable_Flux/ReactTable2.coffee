  ...
  deleteModel:(model)->
    ...
    model.destroy
      ...
      error:(msg)->
        er = msg
    er
  saveModel:(model,props,success,fail)->
    that = @
    isNew = model.isNew()
    model.save props,
      success:->
      ...
      error:(state,exception)->
        fail(exception.responseJSON.ExceptionMessage)
  showModal:(type,model)->
    props =
      componentDidMount:this.options.componentDidMount
      componentDidUpdate:this.options.componentDidUpdate
    switch type
      when "add"
        m = new @collection.model()
        ReactDOM.render <ReactModal {...props} model={m} action={type} saveModel={@saveModel.bind(@)} />,@modalContainer[0]
      when "edit"
        ReactDOM.render <ReactModal {...props} model={model} action={type} saveModel={@saveModel.bind(@)} />,@modalContainer[0]
      when "detail"
        ReactDOM.render <ReactModal  {...props} model={model} action={type} saveModel={@saveModel.bind(@)} />,@modalContainer[0]
  render:->
    ReactDOM.render <ReactTable {...@options } showModal={@showModal.bind(@)}  deleteModel={@deleteModel} getSortList={@getSortList} />,@el

DateTimeCellMixin =
  componentWillUpdate:(nextProps,nextState)->
    el = $(@getDOMNode())
    modalBody = $(React.findDOMNode(@refs.modalBody))
    schema = @props.collection?.model::schema ? @props.model.schema
    ...
    unless e?.isDefaultPrevented?() is true
      switch command
        when "add"
          @props.showModal("add")
        when "edit"
          @props.showModal("edit",model)
        when "detail"
          @props.showModal("detail",model)
        when "delete"
              ...
              <thead>
                {
                    ...
                    columns=for k,v of @props.collection.model::schema when v.visible isnt false
                              <th style={{whiteSpace:"nowrap"}} ref={"th_#{k}"} onClick={@columnHeaderClickHandle.bind(@,k)}>
                                {v.title}
                                {
...
ReactModal = React.createClass
  ...
  getInitialState:->
    modalFormValues: $.extend true, {}, @props.model.attributes
    showModal:true
        ...
        when "fileinput"
          fuOpt =
            ...
            chooseFile:(files,mill)->
              that.refs.fileInputText.value = files[0].name
            beforeUpload:(files,nill)->
            uploadSuccess:(resp)->
              debugger
              that.onModalFieldValueChange(model,key,resp.url)
              that.setState fileUploadSuccess:true
            uploadFail:(resp)->
              debugger
            param:
          ...
          <div className="input-group" ref={ref}>
              ...
              <input type="text" ref="fileInputText" className="form-control"/>
              <span className="input-group-addon">
                <FileUpload options={fuOpt} style={{height:20}} >
              ...
              <Overlay show={@state.fileUploadSuccess} target={=>ReactDOM.findDOMNode(@refs[ref])} container={@}  placement="top">
                      <Popover style={{zIndex:99999}}>文件上传成功</Popover>
              </Overlay>
          </div>
  ...
  onModalFieldValueChange:(model,key,e)->
    if model.schema[key].type.toLowerCase() is "fileinput"
      value = e
    else
      value = if model.schema[key].type.toLowerCase() is "checkbox" then (if e.target.checked is true then "1" else "0") else e.target.value
    ...
    @setState modalFormError:error
  render:->
    if @props.action is "detail"
      fieldsetProps = disabled:"disabled"
    else
      fieldsetProps = {}
    <Modal show={@state.showModal} onHide={@hideModalHandle} bsSize="large" backdrop={if @props.action is "detail" then true else "static"}>
      <Modal.Header closeButton>
      ...
      <Modal.Footer>
        {
          if @props.action in ["add","edit"]
            <div>
                <Button bsStyle="primary" ref="saveButton" onClick={@saveButtonHandle}>保存</Button>
        ...
        }
        {
          if @state.serverError
            <Overlay show={true} target={=>ReactDOM.findDOMNode(@refs.saveButton)} container={@}  placement="top">
                  <Popover style={{zIndex:99999}}>{@state.serverError}</Popover>
            </Overlay>
        }
      </Modal.Footer>
    </Modal>
