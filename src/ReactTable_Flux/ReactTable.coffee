...
ReactTable = React.createClass
  ...
  createDateTimePickerControl:->
    ...
    el = $(@getDOMNode())
    for k,v of schema when v.type.toLowerCase() is "datetime"
      el.find(".dtpControl_#{k}").datetimepicker
        format:v.format
        language:"zh-CN"
        weekStart:1
        todayBtn:1
  ...
  getCellContent:(model,key)->
    ...
    if @props.readonly is true
      ...
      isEdit = true
    # isEdit = model is @state.editCell?.model and key is @state.editCell?.key

    switch schema.type.toLowerCase()
      when "text"
        if isEdit
          content = <input style={{height:36,padding:0}} className="form-control" type="text" bsSize="small" value={model.get(key)} autoFocus="true"/>
        else
          content = <span>{model.get(key)}</span>
      when "select"
        if isEdit
          content = <select style={{height:36,padding:0}} className="form-control"type="checkbox" bsSize="small" value={model.get(key)} autoFocus="true">
                      {
      ...
      when "checkbox"
        if isEdit
          content = <input style={{height:20,marginTop:2}} className="form-control" type="checkbox" bsSize="small" checked={model.get(key) is "1"} autoFocus="true"/>
      ...
      when "datetime"
        if isEdit
          content = <div className="input-group input-append date form_datetime" >
                       <input ref="datetimepicker"  className="form-control dtpControl_#{key}" autoFocus="true" value={model.get(key)}  type="text"  readOnly="readonly"/>
                      ...
                      <tr className={if @state.selectedRow is model then "info" else ""}>
                        {
                          for k,v of model.schema
                            cellStyle = {}
                            if model is @state.editCell?.model and k is @state.editCell?.key or v.edit is true
                              if v.edit is true and v.type.toLowerCase() is "datetime"
                                cellStyle.width = 200
                                cellStyle.padding = 1
                              if model is @state.editCell?.model and k is @state.editCell?.key
                                cellStyle.width=@cellWidths[k]
                              <td  style={cellStyle} onClick={@cellClick.bind(@,model,k)}>{@getCellContent(model,k)}</td>
                            else
                              if v.type.toLowerCase() is "checkbox"
                                <td  className="text-center" onClick={@cellClick.bind(@,model,k)}>{@getCellContent(model,k)}</td>
                              else
